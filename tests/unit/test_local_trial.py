...
class LocalTrialHarnessTests(unittest.TestCase):
    def test_local_trial_writes_all_inspectable_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("pipeline_summary: local trial trial_local_ab12cd wrote", feedback_report)
            self.assertNotIn("- passed: local trial", feedback_report)
            ...
            self.assertEqual(outcome["trial_pipeline_status"], "passed")
            self.assertTrue(outcome["trial_pipeline_passed"])
            self.assertEqual(outcome["product_owner_verdict"], "pending")
            self.assertEqual(outcome["stage_label"], "local_trial_artifact_pipeline")
            self.assertEqual(outcome["stage_scope"], "provider_free_mvp_skeleton")
            self.assertIn("user_acceptance", outcome["not_validated"])
            self.assertFalse(outcome["quality_scope"]["real_llm_quality_validated"])
            ...
            patch_review = (
                vault_root / f"_ai_reports/patch-reviews/{result.patch_id}.md"
            ).read_text(encoding="utf-8")
            self.assertIn("artifact_type: patch_review_report", patch_review)
            self.assertIn('trial_id: "trial_local_ab12cd"', patch_review)
            ...
            self.assertIn("does not record formal patch acceptance", patch_review)
            draft = (
                vault_root
                / "_ai_suggestions/blog-drafts/draft_trial_local_ab12cd/draft.md"
            ).read_text(encoding="utf-8")
    ...
    def test_invalid_extraction_fails_safely_and_persists_run_log(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(outcome["trial_pipeline_status"], "failed")
            self.assertFalse(outcome["trial_pipeline_passed"])
            self.assertEqual(outcome["product_owner_verdict"], "pending")
            self.assertIn("local trial trial_local_ab12cd wrote 3 artifacts", outcome["pipeline_summary"])
