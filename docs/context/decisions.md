...
### 2026-05-28 — Use a source-based alpha package for trial distribution

- Decision: The first small-user trial distribution is a source-based alpha
  package generated by `scripts/package-trial-client-alpha.py`. The package
  includes Python source, built React frontend assets, Win11 launchers,
  trial-client docs, `START_HERE.md`, and a
  `TRIAL_RELEASE_MANIFEST.json`.
- Reason: The project needs realistic user feedback on core extraction quality
  before adding more core functionality or investing in a native installer.
  A repeatable source package is enough for a small alpha group while keeping
  the workflow reviewable and reversible.
- Alternatives: Ship only repository checkout instructions; create a GitHub
  Release immediately; build a native Windows installer now.
- Risks: Trial users still need Python and first-run dependency installation.
  The alpha package must not be mistaken for a stable release.
- Follow-up: Consider a signed native installer, checksums, and release
  publishing only after alpha feedback validates the client workflow and core
  extraction quality.
