...
Provider decisions JSON input is complete.

## Current Focus

Prepare the escalation and execution plan for the first real provider integration from a provider-neutral skeleton baseline. The current skeleton keeps real provider calls disabled, avoids SDK dependencies, avoids API key reads, and limits future first-provider scope to `extract_units`. Provider adapters return typed response/error envelopes; concrete provider adapters now have a typed provider execution request carrying both the provider-neutral request and rendered prompt package; application pipelines can build provider-neutral requests from ingested Markdown, render provider-neutral prompt packages, orchestrate prompt-aware provider-boundary execution, validate provider requests against model policy before execution, validate structured output, map provider request id, retry count, token usage, and prompt hash into typed run-log context, parse provider decision JSON into typed readiness decisions, assess whether required real-provider integration decisions are complete before implementation, render readiness reports and escalation request drafts for review without reading API keys or calling providers, and expose those diagnostics through CLI commands; storage adapters persist `_ai_runs`, `_ai_suggestions`, and `_ai_reports`; and formal vault mutation remains out of scope.
