...
OpenAI manual live-smoke execution path is implemented; actual live smoke remains unrun.

## Current Focus

The OpenAI official SDK is recorded as a production dependency, and the concrete OpenAI adapter exists behind the AI adapter boundary at `src/diamonddust/ai/adapters/openai.py`. The adapter maps `ProviderExecutionRequest` into an OpenAI Responses-style structured-output request shape, maps fake/future provider responses, usage, and errors back into provider-neutral envelopes, and fails closed before API key reads or network execution unless the live-smoke execution path supplies all one-smoke approvals. CLI commands expose sanitized OpenAI payload preview, dry-run reporting, an `openai-live-smoke-readiness` report, and a controlled `openai-extract-units` live path. The live path is blocked by default, enforces `gpt-5.5`, `DIAMONDDUST_OPENAI_API_KEY`, the checked-in small fixture essay, timeout 60 seconds, zero retries, disabled fallback, USD 1.00 cost limit, and hash/metadata-only retention, then persists only AI run logs and validated extraction artifacts when executed. Successful typed extraction proposals can now be persisted as `validated_extraction_output` JSON under `_ai_suggestions/extractions/`; local trial run logs point to that artifact before downstream report/outcome artifacts, while failed or source-mismatched outputs remain represented by run-log hashes and validation errors only. The provider decision set distinguishes API key environment variable name approval from API key value reading approval and separates prompt, source body, and output schema externalization decisions. The product owner has approved exactly one future manual OpenAI `extract_units` live smoke, but this implementation task did not run it and does not approve recurring live smoke, real user essay externalization, provider-side tools, raw provider request/response persistence, patch acceptance, formal apply, or publication. CI remains provider-free by default and does not require or mention the OpenAI API key. The completed implementation plans are `docs/exec-plans/completed/2026-05-21-first-openai-adapter-pre-live-smoke.md`, `docs/exec-plans/completed/2026-05-22-openai-live-smoke-readiness.md`, `docs/exec-plans/completed/2026-05-22-validated-extraction-artifact.md`, and `docs/exec-plans/completed/2026-05-23-openai-live-smoke-execution-path.md`; the future live-smoke execution plan is blocked at `docs/exec-plans/blocked/2026-05-23-first-openai-manual-live-smoke.md`.
...
- Next likely stage: product-owner review of the recorded live-smoke decision package, then an explicit instruction to run the blocked one-manual-live-smoke execution plan if desired.
