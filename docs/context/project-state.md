...
DeepSeek provider adapter is implemented behind the AI adapter boundary. After JSON mode output-instruction hardening, a controlled DeepSeek fixture live smoke passed typed extraction validation and produced a validated extraction artifact under `_ai_suggestions/extractions/` plus an AI run log under `_ai_runs/`. The provider-neutral extraction contract now includes `source_context` for source/article-level domain, background, main content, scope, shape, and source refs, so ordinary units no longer carry whole-note summary responsibility. It also includes DiamondDust's knowledge language policy: generated user-facing knowledge fields should be Simplified Chinese, original evidence should stay in its source language, and machine fields remain English/ASCII. Newly generated artifact timestamps use UTC+8 ISO 8601 strings with `+08:00`; historical fixtures and artifacts with `Z` timestamps remain tolerated. A local trial client now exists for small-user real-note feedback before more core workflows expand, with DeepSeek-V4 model presets, local API key setup, configurable trial workspaces, browser Markdown import, historical artifact loading/deletion, clearer unit review cards, separate source context display, Win11 launcher scripts, and a React/Vite frontend source tree for the longer-lived client surface. The React frontend now has parity with the embedded fallback page for runtime controls, feedback capture, boundary display, and artifact path display. Trial-client run settings for selected model, timeout seconds, max output tokens, and per-run cost limit are persisted locally in `.diamonddust-trial/trial-client-settings.json` and reused as future run defaults. Trial-client API key input, saving, and loading trim surrounding whitespace so copied DeepSeek keys and existing quoted local secret files do not cause false 401s. DeepSeek adapter malformed JSON errors call out provider output truncation when `finish_reason == "length"`. A repeatable source-based alpha distribution package can now be generated with `scripts/package-trial-client-alpha.py`; it includes built frontend assets, Win11 launchers, first-run docs, and a manifest while excluding secrets, `knowledge-vault/`, `.git/`, `.venv/`, `.diamonddust-trial/`, `node_modules/`, and generated provider artifacts. The Win11 launchers now prefer `py -3.13`/`py -3.12`/`py -3.11`/`py -3` before `python`, keep failure windows open, choose an available local port from 8765 through 8775 unless `DIAMONDDUST_TRIAL_PORT` is set, pass a package-local secrets file at `.diamonddust-trial/secrets/provider-secrets.env`, and write diagnostics to `.diamonddust-trial/logs/trial-client-launch.log`. The next DeepSeek product risk is extraction quality on product-owner-approved non-sensitive real notes, not basic provider connectivity, JSON shape, or local trial-client distribution.

## Current Focus

The OpenAI official SDK is recorded as a production dependency, and the concrete OpenAI adapter exists behind the AI adapter boundary at `src/diamonddust/ai/adapters/openai.py`. A second concrete DeepSeek adapter now exists at `src/diamonddust/ai/adapters/deepseek.py` and reuses the existing OpenAI SDK dependency through DeepSeek's documented OpenAI-compatible API. The OpenAI adapter maps `ProviderExecutionRequest` into an OpenAI Responses-style structured-output request shape; the DeepSeek adapter maps `ProviderExecutionRequest` into a DeepSeek Chat Completions JSON Output request shape using `response_format={"type": "json_object"}`, `max_tokens`, and DeepSeek-specific JSON mode message shaping that remains contained inside the adapter. Both adapters return provider-neutral response/error envelopes and fail closed before API key reads or network execution unless explicit real-run approvals are present. CLI commands expose sanitized OpenAI and DeepSeek payload previews, dry-run reporting, controlled extract commands, the OpenAI live-smoke readiness report, and a local `trial-client` browser UI for DeepSeek real-note extraction trials. Successful typed extraction proposals can now be persisted as `validated_extraction_output` JSON under `_ai_suggestions/extractions/`; local trial run logs point to that artifact before downstream report/outcome artifacts, while failed outputs remain represented by run-log hashes and validation errors only. The application provider handoff now treats the request payload `source_input_id` as authoritative top-level lineage, binds it and `source_context.source_input_id` before typed validation, and still rejects any unit/source-context source refs that do not preserve that request-bound source identity. The prompt renderer now also requires `source_context`, provides a request-derived `unit_id_prefix`, prohibits whole-note summary and `raw_essay` provider unit candidates, gives explicit instructions that every unit candidate must include a non-empty id, enum-as-string guidance, and language policy instructions for Chinese user-facing knowledge text. Typed validation diagnostics identify invalid candidate indexes. The trial client surfaces non-empty real-note runs with zero non-raw knowledge unit candidates as `failed_empty_extraction`, renders source context separately from unit candidates, keeps source refs behind expandable evidence panels, can load prior versions without calling a provider, can import Markdown into the active trial workspace, persists local run settings, and can serve a built React frontend through `--frontend-dist` while preserving the embedded fallback page. The provider decision set distinguishes API key environment variable name approval from API key value reading approval and separates prompt, source body, and output schema externalization decisions. The product owner has approved exactly one future manual OpenAI `extract_units` live smoke, and DeepSeek fixture/live trial calls have been run for provider evaluation only; neither provider path approves recurring live smoke, provider-side tools, raw provider request/response persistence, patch acceptance, formal apply, or publication. CI remains provider-free by default and does not require or mention provider API keys. The completed implementation plans are `docs/exec-plans/completed/2026-05-21-first-openai-adapter-pre-live-smoke.md`, `docs/exec-plans/completed/2026-05-22-openai-live-smoke-readiness.md`, `docs/exec-plans/completed/2026-05-22-validated-extraction-artifact.md`, `docs/exec-plans/completed/2026-05-23-openai-live-smoke-execution-path.md`, `docs/exec-plans/completed/2026-05-23-deepseek-provider-adapter.md`, `docs/exec-plans/completed/2026-05-25-provider-source-context-binding.md`, `docs/exec-plans/completed/2026-05-25-extraction-unit-id-prompt-hardening.md`, `docs/exec-plans/completed/2026-05-25-deepseek-json-mode-output-instructions.md`, `docs/exec-plans/completed/2026-05-26-knowledge-language-policy.md`, `docs/exec-plans/completed/2026-05-27-trial-client.md`, `docs/exec-plans/completed/2026-05-28-trial-client-product-feedback.md`, `docs/exec-plans/completed/2026-05-28-trial-client-alpha-win11.md`, `docs/exec-plans/completed/2026-05-28-source-context-extraction-contract.md`, and `docs/exec-plans/completed/2026-06-01-trial-client-run-settings.md`; the future OpenAI live-smoke execution plan is blocked at `docs/exec-plans/blocked/2026-05-23-first-openai-manual-live-smoke.md`.
